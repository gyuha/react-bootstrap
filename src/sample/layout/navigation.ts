...
export type SampleAccordionNavValue = 'pages' | 'auth' | 'errors';
type SampleAccordionNavI18nKey = 'pages' | 'auth' | 'errors';
type SampleNavI18nKey = SampleNavLinkI18nKey | SampleAccordionNavI18nKey;
export type SampleNavTitleI18nKey = `nav.${SampleNavI18nKey}.title`;
export type SampleNavDescriptionI18nKey = `nav.${SampleNavI18nKey}.description`;
export type SamplePagesChildRouteGroup = Exclude<SampleAccordionNavValue, 'pages'>;
...
export interface SampleSecondLevelAccordionNavItem
  extends SampleNavLabelI18nKeys<Exclude<SampleAccordionNavI18nKey, 'pages'>> {
  kind: 'accordion';
  value: Exclude<SampleAccordionNavValue, 'pages'>;
  icon: LucideIcon;
  items: readonly SampleNavItem[];
}

export interface SamplePagesAccordionNavItem
  extends SampleNavLabelI18nKeys<Extract<SampleAccordionNavI18nKey, 'pages'>> {
  kind: 'accordion';
  value: Extract<SampleAccordionNavValue, 'pages'>;
  icon: LucideIcon;
  items: readonly (SampleNavItem | SampleSecondLevelAccordionNavItem)[];
}

export type SampleAccordionNavItem =
  | SamplePagesAccordionNavItem
...
} as const satisfies SampleSecondLevelAccordionNavItem;
...
const samplePagesChildRouteGroups = [
  sampleAuthMenuItem,
  sampleErrorsMenuItem,
] as const satisfies readonly SampleSecondLevelAccordionNavItem[];

const samplePagesChildAccordionValues = [
  sampleAuthMenuItem.value,
  sampleErrorsMenuItem.value,
] as const satisfies readonly SamplePagesChildRouteGroup[];

export const samplePagesMenuItem = {
  kind: 'accordion',
  value: 'pages',
  ...getSampleNavLabelI18nKeys('pages'),
  icon: PanelsTopLeft,
  items: [...sampleNavItems, ...samplePagesChildRouteGroups],
} as const satisfies SamplePagesAccordionNavItem;

const sampleAccordionNavValues = [
  samplePagesMenuItem.value,
  ...samplePagesChildAccordionValues,
...
export function getSamplePagesChildRouteGroup(
  pathname: string
): SamplePagesChildRouteGroup | undefined {
  const childRouteGroup = samplePagesChildRouteGroups.find((group) =>
    group.items.some((item) => isSampleNavItemActive(pathname, item))
  );

  return childRouteGroup?.value;
}

export function isSampleAuthChildRoute(pathname: string): boolean {
  return getSamplePagesChildRouteGroup(pathname) === sampleAuthMenuItem.value;
}

export function isSampleErrorChildRoute(pathname: string): boolean {
  return getSamplePagesChildRouteGroup(pathname) === sampleErrorsMenuItem.value;
}

export function getSampleRouteOpenAccordionGroups(pathname: string): SampleAccordionNavValue[] {
  const childRouteGroup = getSamplePagesChildRouteGroup(pathname);

  if (childRouteGroup) {
    return [samplePagesMenuItem.value, childRouteGroup];
  }

  const hasPagesLinkMatch = sampleNavItems.some((item) => isSampleNavItemActive(pathname, item));

  return hasPagesLinkMatch ? [samplePagesMenuItem.value] : [];
}
...
export function getSampleSidebarOpenAccordionGroups(
  ...
  const routeChildGroup = routeOpenGroups.find(isSamplePagesChildAccordionValue);
  ...
  if (routeChildGroup) {
    for (const childGroup of samplePagesChildAccordionValues) {
      if (childGroup !== routeChildGroup) {
        nextOpenGroups.delete(childGroup);
      }
    }
  }
...
function isSamplePagesChildAccordionValue(value: string): value is SamplePagesChildRouteGroup {
  return samplePagesChildAccordionValues.some((childGroup) => childGroup === value);
}
